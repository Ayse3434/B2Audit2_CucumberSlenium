...
import com.devbook.pages.PostsPage;
...
public class NavigationMenu_StepDefs {
    ...
    public void the_user_should_be_able_to_see_text(String headerText) {
        String actualHeaderText= dashboardPage.getTextHeader(headerText);
        Assert.assertEquals("Verify that header text is displayed",headerText,actualHeaderText);
    }

    @Then("The welcome message contains {string} text")
    public void the_welcome_message_contains_text(String headerText) {
        String actualHeaderText= dashboardPage.bootDeveloperText.getText();
        Assert.assertEquals("Verify that header text is displayed",headerText,actualHeaderText);
    }
}
